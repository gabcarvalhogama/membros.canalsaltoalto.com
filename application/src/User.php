...
");
		...
		public function getUsers($limit = null) {
		    ...
		    $sql = DB::open()->prepare("
				...
				u.zipcode, 
				u.address_state, 
				(SELECT state FROM csa_states WHERE idstate = u.address_state LIMIT 1) as address_state_name,
				u.address_city, 
				(SELECT city FROM csa_cities WHERE idcity = u.address_city LIMIT 1) as address_city_name,
				u.address, 
				u.address_number, 
		...
		public function getAllUsersWithLastSubscription($limit = null) {
		    ...
		    $sql = DB::open()->prepare("
		        SELECT 
					...
					m.membership_title,
					(SELECT COUNT(*) FROM csa_companies c WHERE c.iduser = u.iduser) AS company_counter,
					CASE WHEN um.iduser IS NOT NULL THEN '1' ELSE '0' END AS is_member
					FROM csa_users u
					LEFT JOIN (
						SELECT *
						FROM (
							SELECT um.*,
								ROW_NUMBER() OVER (PARTITION BY um.iduser ORDER BY um.ends_at DESC) AS rn
							FROM csa_users_memberships um
							WHERE um.status = 'paid'
						) t
						WHERE t.rn = 1
					) um ON um.iduser = u.iduser
					LEFT JOIN csa_memberships m ON m.membership_id = um.membership_id
