		...
		$router->mount("/tutorials", function() use ($router){
			...
			$router->post("/new", function(){
				if(empty($_POST["tutorial_title"])){
					...
					if($Tutorial->create($_POST["tutorial_title"], ($_POST["tutorial_video_url"] ?? null), $_POST["tutorial_content"], intval($_POST["tutorial_order"]), (User::getUserIdByEmail2($_SESSION["csa_email"])), $tutorial_status, $tutorial_publish_date))
						die(json_encode(["res" => 1]));
					else
			...
			$router->post("/edit/{tutorial_id}", function($tutorial_id){
				...
				if(empty($_POST["tutorial_title"])){
					...
					$Tutorial = new Tutorial;
					if($Tutorial->update($tutorial_id, $_POST["tutorial_title"], ($_POST["tutorial_video_url"] ?? null), $_POST["tutorial_content"], intval($_POST["tutorial_order"]), $_POST["tutorial_status"], 
